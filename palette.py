...
def display_hsv_palette(img, l_h, l_s, l_v, u_h, u_s, u_v):
    ...
    draw_text_with_semi_transparent_bg(img, "Press 'S' to save | Press 'I' for RGB Input | Press 'ESC' to exit", (20, 280), font_scale=0.6, color=text_color)
...
def main():
    img_hsv = np.zeros((300, 650, 3), np.uint8)
    img_rgb = np.zeros((300, 650, 3), np.uint8)
